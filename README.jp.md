...
- **プロパティ**: 利用可能なプロパティのうち任意の部分集合を各化合物に表示
  ...
  マージすることもできる。浮動小数点値は既定で有効数字3桁表示だが、`MW`
  だけは常に小数点以下2桁固定で表示する（例: `180.16`、分子量の慣例的な
  表記に合わせている）
...
- **フォントサイズ**: `--font-size`（ポイント単位、既定7、化合物名はさらに
  2pt大きく描画される）で化合物名・プロパティのテキストサイズを制御できる
  （HTML・PDF両方に適用）
- **セル間隔**: `--cell-gap`（既定8、HTML はピクセル・PDFはポイント）で
  グリッドセル同士の間隔を制御できる。各セルには薄い枠線も表示される
  （HTML・PDF両方）ため、セルの境界自体も把握しやすい
...
`render_pdf.py`を読む際に知っておくべき落とし穴が一つある: `svglib`は
SVGに書かれた`width="250px"`という宣言を、標準的な96dpi（CSS px）→72dpi
（PDF pt）の変換で解釈するため、250x200pxの構造式は250x200ptではなく
187.5x150ptの`Drawing`としてパースされる。宣言されたピクセル値をそのまま
（パース後の`Drawing`の実際のサイズではなく）スケール計算に使うと、
全ての構造式が意図したサイズの75%で黙って描画されてしまう。
`_svg_native_size`は使い捨ての分子を1つ事前にプローブすることで、
常に正しいスケール係数を使うようにしている。
...
# 文字を小さく/大きく（既定は7pt）、セル間隔を詰める（既定は8）
...
| `--font-size` | プロパティテキストのサイズ（ポイント単位、化合物名はさらに2pt大きく描画）。HTML・PDF両方に適用 | 7 |
| `--cell-gap` | グリッドセル同士の間隔（HTMLはピクセル、PDFはポイント） | 8 |
...
# 原子・結合インデクスを表示、元の向きを保持（自動回転しない）、文字を小さく・セルを詰める
