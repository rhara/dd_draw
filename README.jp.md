...
**mamba/condaが使えない場合（どのプラットフォームでも）:** RDKit・jinja2・
reportlab・svglibは全て通常のPyPIホイールとしても配布されているため、
素のvenvでも動作する:
...
RDKit >= 2024.09.2（2D構造式描画）、Jinja2 >= 3.1（HTMLテンプレート）、
reportlab >= 4.0 + svglib >= 1.5（PDF生成）。JavaScript・CDN・システムの
Cairo/Pango/GTKは一切不要。
